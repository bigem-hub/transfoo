...
function expired(ts) { return Date.now() - ts > 300000; } // 5 min
...
router.post('/', (req, res) => {
  ...
  res.json({ code, deviceName: name, expiresIn: '5m', authorize: '/api/pairing/authorize' });
});
...
router.post('/authorize', (req, res) => {
  ...
  if (expired(sess.created)) { pairCache.delete(code); return res.status(401).json({ error: 'pairing session expired' }); }
